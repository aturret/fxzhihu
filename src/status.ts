import { fixImagesAndLinks, createTemplate, extractReference, FetchError, findVideoUrl } from './lib';
...
export type ContentItem = TextContent | VideoContent | ImageContent;
...
export async function status(id: string, redirect: boolean, json: boolean, env: Env): Promise<string> {
	...
	if (data.origin_pin) {
		...
		originPinResult = originPinTemplate({
			...
			originPinContent: fixImagesAndLinks(data.origin_pin.content_html),
            ...
            originPinVideoHtml: originPinVideoHtml,
		});
	}
	...
	return template({
		...
		originPinResult: originPinResult,
        videoHtml: videoHtml,
	});
}
